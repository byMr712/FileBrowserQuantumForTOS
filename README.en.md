...
Built package **FileBrowser Quantum 2.0.4-beta** for TerraMaster x86_64 NAS (TOS6/TOS7).
...
- **Updated to 2.0.4-beta by:** [Mr712](https://github.com/byMr712?tab=repositories).
- This package is built solely from the original module **1.2.1-stable** packaging and the
  ...
  **Nothing was removed from or added to the original module** — only the FileBrowser Quantum
  application itself was updated to v2.0.4-beta and its configuration adapted.
...
- This package is a **beta** build (2.0.4-beta) from a community member and is **not** an official
...
| Path | Description |
|---|---|
| `FileBrowserQuantum_TOS7_TOS6_2.0.4.0-beta-x86_64.tpk` | Ready package for App Center |
| `FileBrowserQuantum_2.0.4.0_pkg/` | Package tree (payload sources): `config.ini`, `.lang`, `INFO`, `version`, `bin/`, `functions/`, `images/`, `init.d/`, `webui.bz2` |
...
Edit files in `FileBrowserQuantum_2.0.4.0_pkg/` (e.g. `bin/filebrowser.yml` — the volume list,
...
`FileBrowserQuantum_TOS7_TOS6_2.0.4.0-beta-x86_64.tpk` (id, version and platform come from `config.ini`).

All default paths are **relative to the script folder** `tools\`: package tree
`..\FileBrowserQuantum_2.0.4.0_pkg`, result `..\FileBrowserQuantum ... .tpk`.
...
Options: `-PkgDir <path>` (default `..\FileBrowserQuantum_2.0.4.0_pkg`),
...
`-ReleaseTag ''` → `FileBrowserQuantum_TOS7_TOS6_2.0.4.0-x86_64.tpk`).

## Full rebuild from the filebrowser sources

The backend/frontend sources live separately, e.g. `C:\Users\Mr712\Desktop\filebrowser-2.0.4-beta`.
Steps:
...
go build -trimpath -o filebrowserquantum --ldflags="-w -s -X 'github.com/gtsteffaniak/filebrowser/backend/internal/version.CommitSHA=n/a' -X 'github.com/gtsteffaniak/filebrowser/backend/internal/version.Version=2.0.4-beta'" .

# 3) Replace the binary in the package tree
Copy-Item backend\filebrowserquantum .\FileBrowserQuantum_2.0.4.0_pkg\bin\program\filebrowserquantum
...
$b=[IO.File]::ReadAllBytes("$pwd\FileBrowserQuantum_TOS7_TOS6_2.0.4.0-beta-x86_64.tpk")
