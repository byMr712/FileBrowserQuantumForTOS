...
Готовый пакет **FileBrowser Quantum 2.0.4-beta** для x86_64 NAS TerraMaster (TOS6/TOS7).
...
- **Кем обновлён до 2.0.4-beta:** [Mr712](https://github.com/byMr712?tab=repositories).
- Пакет собран исключительно из упаковки оригинального модуля **1.2.1-stable** и исходников
  ...
  **Ничего не было вырезано и не добавлено** — обновлено только само приложение FileBrowser
  Quantum до v2.0.4-beta и адаптирована его конфигурация.
...
- Пакет — **бета**-сборка (2.0.4-beta) от участника сообщества и **не является** официальным
...
| Путь | Описание |
|---|---|
| `FileBrowserQuantum_TOS7_TOS6_2.0.4.0-beta-x86_64.tpk` | Готовый пакет для установки в App Center |
| `FileBrowserQuantum_2.0.4.0_pkg/` | Дерево пакета (исходники payload): `config.ini`, `.lang`, `INFO`, `version`, `bin/`, `functions/`, `images/`, `init.d/`, `webui.bz2` |
...
Правьте файлы в `FileBrowserQuantum_2.0.4.0_pkg/` (например `bin/filebrowser.yml` — список томов,
...
`FileBrowserQuantum_TOS7_TOS6_2.0.4.0-beta-x86_64.tpk` (id, версия и платформа — из `config.ini`).

Все пути по умолчанию — **относительные, от папки скрипта** `tools\`: дерево пакета
`..\FileBrowserQuantum_2.0.4.0_pkg`, результат `..\FileBrowserQuantum ... .tpk`.
...
Опции: `-PkgDir <путь>` (по умолчанию `..\FileBrowserQuantum_2.0.4.0_pkg`),
...
Исходники backend/frontend лежат отдельно, например `C:\Users\Mr712\Desktop\filebrowser-2.0.4-beta`.
Порядок:
