...
Built package **FileBrowser Quantum 2.0.1-beta** for TerraMaster x86_64 NAS (TOS6/TOS7).
...
- **Updated to 2.0.1-beta by:** [Mr712](https://github.com/byMr712?tab=repositories).
- This package is built solely from the original module **1.2.1-stable** packaging and the
  ...
  **Nothing was removed from or added to the original module** — only the FileBrowser Quantum
  application itself was updated to v2.0.1-beta and its configuration adapted.
...
- This package is a **beta** build (2.0.1-beta) from a community member and is **not** an official
...
Edit files in `FileBrowserQuantum_2.0.1.0_pkg/` (e.g. `bin/filebrowser.yml` — the volume list,
...
`FileBrowserQuantum_TOS7_TOS6_2.0.1.0-beta-x86_64.tpk` (id, version and platform come from `config.ini`).

All default paths are **relative to the script folder** `tools\`: package tree
`..\FileBrowserQuantum_2.0.1.0_pkg`, result `..\FileBrowserQuantum ... .tpk`.
...
Options: `-PkgDir <path>` (default `..\FileBrowserQuantum_2.0.1.0_pkg`),
...
The backend/frontend sources live separately, e.g. `C:\Users\Mr712\Desktop\filebrowser-2.0.1-beta`.
Steps:
